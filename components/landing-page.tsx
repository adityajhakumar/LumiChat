...
import { ArrowRight, Check, X, DollarSign, TrendingDown, Sparkles, FileText, Image, Brain, Code2, Layers, Zap, Lock, Users } from 'lucide-react';

export default function LumiChatsLanding({ onEnterApp }) {
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="relative p-10 bg-gradient-to-br from-orange-950/20 via-purple-950/10 to-orange-900/10 rounded-3xl border-2 border-orange-500">
              ...
              <div className="text-center mt-4">
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-8 text-left">
                  {[
                    { title: "1M Tokens Daily", desc: "Massive context for complex projects" },
                    { title: "Unlimited Uploads", desc: "PDFs, code, images — everything" },
                    { title: "Image Analysis", desc: "Understand diagrams, charts, screenshots" },
                    { title: "Code Execution", desc: "Run, test, debug in real-time" },
                    { title: "Long Context Memory", desc: "Remembers your entire conversation" },
                    { title: "Fast Processing", desc: "Lightning-quick responses" }
                  ].map((item, i) => (
                    <div key={i} className="flex items-start space-x-3 p-4 bg-zinc-900/30 rounded-xl border border-zinc-800">
          ...
          <div className="mt-16 p-8 bg-gradient-to-r from-green-950/20 to-emerald-950/10 rounded-2xl border border-green-900/30">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                { days: "8 days", usage: "Light use month", cost: "₹312", save: "₹1,688" },
                { days: "12 days", usage: "Average use month", cost: "₹468", save: "₹1,532" },
                { days: "20 days", usage: "Heavy use month", cost: "₹780", save: "₹1,220" }
              ].map((plan, i) => (
                <div key={i} className={`text-center p-6 bg-zinc-900/50 rounded-xl ${i === 1 ? 'border-2 border-orange-500/30' : ''}`}>
                  <div className="text-3xl font-bold text-orange-400 mb-2">{plan.days}</div>
                  <div className="text-zinc-400 mb-3">{plan.usage}</div>
                  <div className="text-2xl font-semibold text-green-400">{plan.cost}</div>
                  <div className="text-sm text-zinc-500 mt-2">vs ₹2000 subscription</div>
                  <div className="text-xs text-green-400 mt-1 font-semibold">Save {plan.save}</div>
                </div>
              ))}
            </div>
          </div>
